...
import { Card, Row, Col, Tab, Nav, Button, Modal, Table, Badge, Alert } from 'react-bootstrap';
...
const MealPlanResults = ({ userProfile, mealPlan }) => {
  ...
  // Effect to handle save success alert
  useEffect(() => {
    if (saveSuccess) {
  ...
  const handleSavePlan = async () => {
    ...
    try {
      ...
      console.log("Sending data to save:", { userProfile, mealPlan });
      
      const response = await axios.post('/api/saved-plan', { 
        userProfile, 
        mealPlan 
      });
      ...
      if (error.response) {
        ...
        setSaveError(`Server error: ${error.response.status} - ${error.response.data.message || 'Unknown error'}`);
      } else if (error.request) {
        // The request was made but no response was received
        console.error("Error request:", error.request);
        setSaveError("No response received from server. Please check your connection.");
      } else {
    ...
    <div className="meal-plan-results">
      ...
      {showSaveAlert && (
        ...
        >
          ...
          <p>
            Your meal plan has been saved to your account. You can access all your saved meal plans anytime.
          </p>
          <div className="d-flex justify-content-end">
            <Button variant="outline-success" onClick={handleViewSavedPlans}>
              View Saved Plans
            </Button>
          </div>
        </Alert>
