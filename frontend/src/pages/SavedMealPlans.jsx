...
const SavedMealPlans = () => {
  ...
  const fetchSavedPlans = async () => {
    ...
    try {
      ...
      console.log("Fetched saved plans:", plans);
      setSavedPlans(plans);
      setLoading(false);
    } catch (err) {
      ...
      {savedPlans.length === 0 ? (
        <Card className="empty-state">
          <Card.Body className="text-center py-5">
            <FaClipboardList className="empty-state-icon" size={48} />
            <h4 className="mt-3">No Saved Meal Plans</h4>
            <p className="text-muted">You haven't saved any meal plans yet.</p>
            <Link to="/profile">
