...
class DietRecommendationApp:
    ...
    def filter_foods_by_constraints(self, diet_type, meal_type, season, cuisines=None, allergens=None):
        ...
        # Filter by diet type if specified, using hierarchical relationship
        if diet_type:
            diet_type_lower = diet_type.lower()
            
            # Apply hierarchical diet filtering:
            # - Vegans only see vegan foods
            # - Vegetarians see both vegetarian and vegan foods
            # - Non-vegetarians see all foods
            if diet_type_lower == 'vegan':
                diet_mask = filtered_df['diet_type'].str.lower() == 'vegan'
            elif diet_type_lower == 'vegetarian':
                diet_mask = (filtered_df['diet_type'].str.lower() == 'vegetarian') | (filtered_df['diet_type'].str.lower() == 'vegan')
            elif diet_type_lower == 'non-vegetarian' or diet_type_lower == 'non vegetarian':
                # Non-vegetarians can see all foods - no filtering needed
                diet_mask = filtered_df['diet_type'].notna()  # This will be True for all rows with a non-null diet_type
            else:
                # For any other specific diet type, match exactly
                diet_mask = filtered_df['diet_type'].str.lower() == diet_type_lower
            
            # Apply the diet mask
            if diet_mask.sum() > 0:
                filtered_df = filtered_df[diet_mask]
            else:
                # If no foods match, try a more generic approach
                print(f"Warning: No foods found with diet type '{diet_type}'. Showing all foods.")
        ...
        if len(filtered_df) == 0:
            ...
            if diet_type and allergens:
                ...
                # Apply hierarchical diet filtering (same logic as above)
                diet_type_lower = diet_type.lower()
                if diet_type_lower == 'vegan':
                    diet_mask = fallback_df['diet_type'].str.lower() == 'vegan'
                elif diet_type_lower == 'vegetarian':
                    diet_mask = (fallback_df['diet_type'].str.lower() == 'vegetarian') | (fallback_df['diet_type'].str.lower() == 'vegan')
                elif diet_type_lower == 'non-vegetarian' or diet_type_lower == 'non vegetarian':
                    diet_mask = fallback_df['diet_type'].notna()
                else:
                    diet_mask = fallback_df['diet_type'].str.lower() == diet_type_lower
